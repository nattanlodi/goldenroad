...
import { rarityFor, tierFor, yy } from "../../game/helpers";
import { lineAvg, picksToLineup, type Competitor } from "../../game/tournament";
...
/**
 * Coluna de "elencos ao vivo" dos outros times. Cada time é uma LINHA compacta
 * (nome + 5 mini-slots de lane) — 7 linhas cabem na altura do mapa sem scroll.
 * No offline simula o preenchimento dos bots acompanhando o SEU ritmo; no online
 * virão os picks reais via rede.
 */
function LiveRostersColumn({ others, revealed, showRatings }: { others: Competitor[]; revealed: number; showRatings: boolean }) {
  return (
    <div className="flex h-full flex-col rounded-2xl border border-gold/25 p-2.5" style={{ background: "rgba(26,27,31,0.55)" }}>
      <div className="mb-[14px] flex items-center justify-between px-0.5">
        <span className="font-mono text-[9.5px] uppercase tracking-[1.5px] text-muted">Outros times</span>
